...
get_data(Options) ->
  ServiceFilter = haproxy_check_cli:setting(service_filter, Options),
  Username = haproxy_check_cli:setting(username, Options),
  Password = haproxy_check_cli:setting(password, Options),
  Host = haproxy_check_cli:setting(host, Options),
  Port = haproxy_check_cli:setting(port, Options),
  ...
  case Result of
    ok         -> {ok, lists:map( fun(X) -> {<<"attrs">>, Record} = lists:nth(1, X), Record end, Body)};
    {error, Why} -> {error, Why}
  end.
